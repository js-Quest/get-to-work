...
function addEmployee() {
  ...
  Promise.resolve()
    ...
    .then((rolesResult) => {
      ...
      const roles = rolesResult.map((item) => `Title: ${item.title}, Role ID: ${item.id}`)
    ...
    .then((managerResult) => {
      ...
      const managers = managerResult.map((item) => `Manager: ${item.manager_name}, Manager ID: ${item.manager_id}`);
...
function deleteEmployee() {
  sqlString = `SELECT id, first_name, last_name FROM employee`;
  Promise.resolve()
  .then(()=> {
    return new Promise((resolve,reject)=>{
      db.query(sqlString, (err,result)=>{
